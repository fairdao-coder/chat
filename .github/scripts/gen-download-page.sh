...
#
# 用法:
#   gen-download-page.sh <download-dir> <version> <short-sha> [site-base]
#
# 在 <download-dir> 下產出 index.html，只列出目錄中「確實存在」的 APK，
# 避免產出指向不存在檔案的 404 下載連結。
#
# 每個下載項目都內嵌其「絕對下載網址」的 QR code，方便手機直接掃描安裝；
# 頁尾另提供「聊天應用」與「配置生成器」的 QR code 直達入口。
# QR code 在瀏覽器端用純 JS 生成（MIT 協議的 qrcodejs，已內聯），無外部依賴。
#
# 注意: 本腳本必須以 LF 行尾執行。CI 中會先做 CR 清理再呼叫它。
set -euo pipefail

dir="${1:?usage: gen-download-page.sh <download-dir> <version> <short-sha> [site-base]}"
version="${2:?missing version}"
sha="${3:?missing short sha}"

# 站点 base（頁面部署後的根 URL），預設 GitHub Pages 專案頁路徑。
# 部署到自定義網域或 user/org 頁時，請傳入對應 base 並以 "/" 結尾。
site_base="${4:-https://servestatic.github.io/Chat/}"
...
# 檔案不存在時靜默跳過，不產生壞連結；存在時輸出含 QR code 的卡片。
card() {
  local file="$1" title="$2" desc="$3" primary="${4:-no}"
  [ -f "$file" ] || return 0
  local size cls="" abs
  size="$(human_size "$file")"
  abs="${site_base}download/${file}"
  [ "$primary" = "yes" ] && cls=" card--primary"
  cat <<HTML
        <div class="card${cls}">
          <a class="card__main" href="${file}" download>
          ...
          <div class="card__qr"><div class="qr" data-url="${abs}"></div></div>
        </div>
HTML
}
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .list { display: grid; gap: 14px; margin: 0 0 24px; }
  .card {
  ...
  .card__main:hover { background: rgba(37,99,235,.06); }
  ...
  .card__qr {
    ...
    padding: 0 16px 16px; border-top: 1px dashed var(--border);
  }
  .card__qr .qr { display: flex; justify-content: center; margin-top: 14px; }
  .card__qr .qr img, .card__qr .qr canvas {
    background: #fff; padding: 8px; border-radius: 10px;
  }
  .qr-section { margin: 8px 0 24px; }
  .qr-section h2 { font-size: 18px; margin: 0 0 4px; letter-spacing: -.01em; }
  .qr-section .sub { margin-bottom: 12px; }
  .qr-grid {
  ...
  .qr-box h3 { margin: 0 0 10px; font-size: 14px; }
  .qr-box .qr { display: flex; justify-content: center; }
  .qr-box .qr img, .qr-box .qr canvas {
    background: #fff; padding: 8px; border-radius: 10px;
  }
  .qr-box a { display: inline-block; margin-top: 10px; color: var(--accent);
...
<body>
  <div class="wrap">
    <header>
      <h1>聊天 App 下載</h1>
      <p class="sub">選擇適合你設備的 Android 安裝包，或掃描下方 QR code 直接安裝。</p>
    </header>
...
HEAD
  ...
  cat <<FOOT
    ...
    <div class="qr-section">
      <h2>掃碼直達</h2>
      <p class="sub">用手機相機或掃描 App 即可開啟對應頁面。</p>
      <div class="qr-grid">
        <div class="qr-box">
          <h3>聊天應用</h3>
          <div class="qr" data-url="${site_base}"></div>
          <a href="${site_base}">開啟</a>
        </div>
        <div class="qr-box">
          <h3>配置生成器</h3>
          <div class="qr" data-url="${site_base}config/"></div>
          <a href="${site_base}config/">開啟</a>
        </div>
      </div>
    </div>
    ...
    <div class="note">
      ...
      選擇「仍要安裝」即可。<br>
      掃描 APK 的 QR code 後，手機瀏覽器會直接下載安裝包，再依上述步驟安裝。
    </div>
...
# QR code 生成器：qrcodejs (MIT License) 內聯，頁面離線也能用、無外部依賴。
# 遍歷所有 .qr 元素，依其 data-url 屬性生成 QR code。
cat <<'QRJS' >> index.html
<script>
...
(function(){
  document.querySelectorAll('.qr').forEach(function(el){
    ...
    if(!url)return;
    try{
      new QRCode(el,{text:url,width:170,height:170,correctLevel:QRCode.CorrectLevel.H});
    }catch(e){
      el.innerHTML='<span style="color:var(--muted);font-size:12px">QR 生成失敗</span>';
    }
  });
})();
</script>
